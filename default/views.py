...
def close_foods_helper(request):
    ...
    if request.method == 'POST' and request.is_ajax():
        ...
        #     'data':data
        # })
        data=list(filter(lambda x: user_location(float(user_lat),float(user_lng))-meal_location(x.cord_lat,x.cord_lng)<1000, Meals.objects.all()))

        paginator = Paginator(data, 1)  # Show 5 per page
...
def orders(request, id):
    ...
    if user.user_type == 'User':
        ...
        context['orders_checker'] = True if Ordering.objects.filter(customer=request.user).last() is not None else False
        ...
        context['orders_checker'] = True if Ordering.objects.filter(author_of_meal=request.user).last() is not None else False
...
def time_day(request):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        if category != 'All':
            context['meals'] = Meals.objects.filter(category=category).all()
        else:
            context['meals'] = Meals.objects.all()
